...
def delete_category(treeview):
    try:
        ...
        cursor.execute('DELETE FROM cat_data WHERE id=%s', (id,))
        connection.commit()
...
def add_category(id, name , description,treeview):
    if id=='' or name=='' or description=='':
        ...
        try:
            cursor.execute('USE dbmsProj')
            cursor.execute('CREATE TABLE IF NOT EXISTS cat_data (id INT PRIMARY KEY, name VARCHAR(100), description TEXT)')
            cursor.execute('SELECT * FROM cat_data WHERE id=%s',id)
            if cursor.fetchone():
                messagebox.showerror('Error', 'Id already exists.')
                return
            cursor.execute('INSERT INTO cat_data VALUES(%s,%s,%s)', (id,name,description))
            connection.commit()
...
def category_form(window):
    ...
    category_frame.place(x=200, y=110)
    ...
    add_button=Button(button_frame, text='Add',font=('times new roman',14),width=8, cursor='hand2',fg='white', 
                         bg='#0f4d7d', command=lambda : add_category(id_entry.get(), category_name_entry.get(),description_text.get(1.0,END).strip(),treeview))
    ...
    clear_button=Button(button_frame, text='Clear',font=('times new roman',14),width=8, cursor='hand2',fg='white', 
                         bg='#0f4d7d', command=lambda:clear(id_entry,category_name_entry,description_text))
