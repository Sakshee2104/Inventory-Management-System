...
def delete_supplier(invoice,treeview):
    ...
    try:
        ...
        cursor.execute('DELETE FROM sup_data WHERE invoice=%s',invoice)
        connection.commit()
...
def search_supplier(search_value,treeview):
    if search_value=='':
        ...
        try:
            cursor.execute('USE dbmsProj')
            cursor.execute('SELECT * FROM sup_data WHERE invoice=%s', search_value)
...
def update_supplier(invoice, name, contact, description, treeview):
    ...
    try:
        ...
        if not cursor or not connection:
            return
        cursor.execute('USE dbmsProj')
        cursor.execute('SELECT * FROM sup_data WHERE invoice=%s', invoice)
        ...
        cursor.execute('UPDATE sup_data SET name=%s, contact=%s, description=%s WHERE invoice=%s', (name,contact,description,invoice))
        connection.commit()
...
def add_supplier(invoice, name, contact, description, treeview):
    if invoice=='' or name=='' or contact=='' or description=='':
        ...
        try:
            ...
            cursor.execute('CREATE TABLE IF NOT EXISTS sup_data (invoice INT PRIMARY KEY, name VARCHAR(100), contact VARCHAR(20), description TEXT)')
            ...
            cursor.execute('INSERT INTO sup_data VALUES(%s,%s,%s,%s)', (invoice,name,contact,description))
            connection.commit()
...
def supplier_form(window):
    ...
    add_button=Button(button_frame, text='Add',font=('times new roman',14),width=8, cursor='hand2',fg='white', 
                         bg='#0f4d7d', command=lambda: add_supplier(invoice_entry.get(),name_entry.get(),contact_entry.get(),description_text.get(1.0,END).strip(), treeview))
    add_button.grid(row=0, column=0, padx=20)

    update_button=Button(button_frame, text='Update',font=('times new roman',14),width=8, cursor='hand2',fg='white', 
                         bg='#0f4d7d', command=lambda :update_supplier(invoice_entry.get(),name_entry.get(),contact_entry.get(),description_text.get(1.0,END).strip(), treeview))
    ...
    clear_button=Button(button_frame, text='Clear',font=('times new roman',14),width=8, cursor='hand2',fg='white', 
                         bg='#0f4d7d', command=lambda : clear(invoice_entry,name_entry,contact_entry,description_text,treeview))
    clear_button.grid(row=0, column=3)

    right_frame=Frame(supplier_frame, bg='white')
    right_frame.place(x=520, y=95, width=500, height=355)
