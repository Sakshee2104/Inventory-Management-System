...
def search_product(search_combobox, search_entry,treeview):
    if search_combobox.get()=='Search By':
        ...
        cursor.execute(f'SELECT * FROM prod_data WHERE {search_combobox.get()}=%s', search_entry.get())
    ...
    if ans:
        ...
        try:
            ...
            if not cursor or not connection:
                return
            cursor.execute('USE dbmsProj')
            cursor.execute('DELETE FROM prod_data WHERE id=%s',id)
            connection.commit()
            treeview_data(treeview)
...
def update_product(category, supplier, name, price,discount, quantity,status,treeview):
    ...
    if not cursor or not connection:
        return
    cursor.execute('USE dbmsProj')
    cursor.execute('SELECT * FROM prod_data WHERE id=%s', id)
    ...
    cursor.execute('UPDATE prod_data SET category=%s, supplier=%s, name=%s, price=%s, discount=%s, discounted_price=%s, quantity=%s, status=%s WHERE id=%s', (category, supplier, name, price, discount,discounted_price,quantity,status,id))
    connection.commit()
...
def fetch_supp_category(category_combobox,supplier_combobox):
    category_option=[]
    supplier_option=[]
    cursor,connection=connect_database()
    if not cursor or not connection:
        return
    cursor.execute('USE dbmsProj')
    cursor.execute('SELECT name FROM cat_data')
    ...
    cursor.execute('SELECT name FROM sup_data')
    names=cursor.fetchall()
    if len(names)>0:
        supplier_combobox.set('Select')
        for name in names:
...
def add_product(category, supplier, name, price,discount,quanity,status,treeview):
    if category=='Empty':
        messagebox.showerror('Error', 'Please add categories.')
    elif supplier=='Empty':
        messagebox.showerror('Error', 'Please add suppliers.')
    elif category=='Select' or supplier=='Select' or name=='' or price=='' or quanity=='' or status=='Select Status':
        ...
        if not cursor or not connection:
            return
        cursor.execute('USE dbmsProj')
        cursor.execute('CREATE TABLE IF NOT EXISTS prod_data (id INT AUTO_INCREMENT PRIMARY KEY, category VARCHAR(100), supplier VARCHAR(100), name VARCHAR(100), price DECIMAL(10,2), quantity INT, status VARCHAR(50))')
        
        cursor.execute('SELECT * FROM prod_data WHERE category=%s AND supplier=%s AND name=%s',(category,supplier,name))
        existing_product = cursor.fetchone()
        if existing_product:
            messagebox.showerror('Error','Product already exists.')
            return
        discounted_price=round(float(price)*(1-float(discount)/100),2)
        cursor.execute('INSERT INTO prod_data (category, supplier,name, price,discount, discounted_price, quantity,status) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)',(category, supplier, name, price,discount, discounted_price, quanity,status))
...
def product_form(window):
    ...
    delete_button=Button(button_frame, text='Delete',font=('times new roman',14),width=8, cursor='hand2',fg='white', 
                         bg='#0f4d7d', command=lambda :delete_product(treeview,category_combobox,supplier_combobox,name_entry, price_entry,quantity_entry,status_combobox,discount_spinbox))
    delete_button.grid(row=0, column=2, padx=10)

    clear_button=Button(button_frame, text='Clear',font=('times new roman',14),width=8, cursor='hand2',fg='white', 
                         bg='#0f4d7d', command=lambda :clear_fields(category_combobox,supplier_combobox,name_entry, price_entry,quantity_entry,status_combobox,discount_spinbox, treeview))
    ...
    treeview=ttk.Treeview(treeview_frame, column=('id','category','supplier','name','price','discount','discounted_price','quantity','status'), show='headings',
    ...
    fetch_supp_category(category_combobox,supplier_combobox)
    treeview_data(treeview)
